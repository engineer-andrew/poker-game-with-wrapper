...
namespace CardGames.Shuffler.Logic.Tests
{
    ...
    public class ShufflerUnitTests
    {
        ...
        public void ShuffleShouldRandomizeADeckOfCards()
        {
            ...
            for (var i = 0; i < 52; i++)
            {
                if (unshuffledDeck[i] == deck.Cards[i] || (i > 0 && deck.Cards[i-1] == unshuffledDeck[i]) || (i < 51 && deck.Cards[i + 1] == unshuffledDeck[i]))
                {
                    numberOfMatches++;
                }
            }
            Assert.IsTrue(numberOfMatches < 5, $"{numberOfMatches} is not less than 5");
        }
    }
}